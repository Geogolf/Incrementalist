...
const updateMessages = {
  "0.5.3": "- Added a bunch of suggestions and qol features<br>- Changed exported files (old ones still work)<br>- Balanced early game and mid-game click walls<br>- A fix for a challenge 4 bug has been added, completions have been reset<br>- A bunch of small tweaks for balancing and visuals<br>- Fixed 5+ bugs",
...
const layers = {
  ...
  "PP": {
    ...
    goalsSac: [0, 0, 0, 0],
    unlocks: ["Prestige Tree and Milestones", "Challenges", "Automate Sacrifice", "Automate Prestige"]
  }
}
...
function runGameTime(active, time) {
  ...
  if (user.pp.milestones < 5) {
    ...
    if ((user.ip.sac.gte(layers.IP.goals[10]) && user.sacrifice.IP.count >= layers.IP.goalsSac[10]) || user.sacrifice.IP.count > layers.IP.goalsSac[10] || user.pp.count > 0 || user.pp.total.gt(0)) {showIdTab("tabScalingb")} else {hideId("tabScalingb")}*/
  }
  ...
  if (user.pp.milestones < 5) {
    ...
    if (user.automation.IncrementE.unlocked) {showId("autoIncrementE"); showId("scalingE")} else {hideId("autoIncrementE"); hideId("scalingE")}
  ...
  //Unlock Scaling
  if (user.pp.milestones < 5) {for (let i=0; i<names.length; i++) {if (user.scaling[names[i]].unlocked) {showId("scaling"+names[i])} else {hideId("scaling"+names[i])}}}
