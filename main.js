...
const layers = {
  "IP": {
    goals: [nd(1000), nd(2500), nd(10000), nd(50000), nd(250000), nd(1e6), nd(5e9), nd(1e13), nd(5e15), nd(1e21), nd(2.1e21), nd(7.5e21), nd(5e32), nd(3.7e37), nd(1e44), nd(2.25e45), nd(4e57), nd(8.5e85), nd(1e100), nd("e1560"), nd("e3161")],
  ...
  "PP": {
    goals: [nd(1), nd(10), nd(50), nd(500)/*, nd(1e35)*/, nd(4e9)],
    goalsSac: [0, 0, 0, 0/*, 2*/, 1],
...
function updatePointDisplay(layer) {
  ...
  if (layer == "IP") {
    ...
    (user.automation.IP.enabled["0"]) ? di("ipSec").textContent = e("d", getEquationIPResult().times(getAutomationRate("IP")).times(multi), 2, 0) : di("ipSec").textContent = e("d", nd(0), 2, 0);
  ...
  if (layer == "PP") {
    ...
    (user.automation.Prestige.enabled["0"]) ? di("ppSec").textContent = e("d", user.pp.lastGain.divide(user.time.lastPrestige/1000), 2, 0) : di("ppSec").textContent = e("d", nd(0), 2, 0);
...
function runGameTime(active, time) {
  ...
  if (user.sacrifice.PP.count == 0) {user.pp.milestones.unlocked = 5}
  if (user.sacrifice.PP.count > 0) {user.pp.milestones.unlocked = 6}
  for (let i=1; i<pm.length; i++) {
    if (user.pp.count >= pm[i].req && user.pp.milestones.unlocked >= i) {user.pp.milestones.count = i}
  ...
  if (user.pp.count > 0 || user.pp.total.gt(0)) {user.ip.infinite = nd("1eeee1")}
...
function checkUnlocks() {
  ...
  if (user.pp.sac.gte(layers.PP.goals[4]) && false) {user.automation.Automation.unlocked = true}
  if (user.pp.sac.gte(layers.PP.goals[4]) && false) {user.automation.Scaling.unlocked = true}
...
function displayUnlocks() {
  ...
  if (user.automation.Automation.unlocked) {showClass("autoAutomationUnlocks")} else {hideClass("autoAutomationUnlocks")}
  if (user.automation.Scaling.unlocked) {showClass("autoScalingUnlocks")} else {hideClass("autoScalingUnlocks")}
