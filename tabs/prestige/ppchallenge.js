const ppChallenge = [
  "null",
  {goals: ["1e153", "1e250", "1e421", "1e799"/*, "1e7333"*/], maxCompletions: 4},
  {goals: ["1e42", "1e73", "1e202"/*, "1e1650"*/], maxCompletions: 3},
  {goals: ["1e93", "1e385"/*, "1e2921"*/], maxCompletions: 2},
  {goals: ["1e105", "1e150"/*, "1e10000"*/], maxCompletions: 2},
  {goals: ["1e182"], maxCompletions: 1},
  {goals: ["1e10000"], maxCompletions: 1}
];
...
function exitPPChallenge(c) {
  let completions = 0;
  if (user.pp.milestones < 600) {if (user.ip.sac.gte(getPPChallengeGoal(c))) {completions = 1}}
...
//Get Data
function getPPChallengeGoal(c, completions) {
  let count = completions || user.pp.challenge[c].count;
  let goal = nd(ppChallenge[c].goals[count]);
  let multi = nd(1);
...
function getPPChallengeReward(c) {
  ...
  if (c == 5) {return nd(user.sacrifice.IP.count+10).log10().minus(1).times(user.pp.challenge[c].count).plus(1).pow(exp)}
  if (c == 6) {return nd(1)}
}
